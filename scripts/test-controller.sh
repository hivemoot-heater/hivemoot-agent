...
run_shutdown_term_stops_job_subshells_case() {
  ...
  # MOCK_DOCKER_WAIT_SLEEP_SECS=30 keeps the job subshell blocked in `docker
  # wait` when SIGTERM arrives.  setsid makes the controller itself the
  # process group leader so that kill -- -$controller_pid during teardown
  # reaps any orphaned mock-wait grandchildren without leaking into later cases.
  setsid env -i \
    PATH="${case_dir}/mock-bin:${PATH}" \
    HOME="${case_dir}/home" \
    MOCK_DOCKER_STATE_DIR="${case_dir}/mock-state" \
    MOCK_DOCKER_WAIT_SLEEP_SECS="30" \
    TARGET_REPO="owner/repo" \
    CONTROLLER_RUN_MODE="once" \
    CONTROLLER_MAX_WORKERS="1" \
    CONTROLLER_WORKSPACE_ROOT="${case_dir}/workspace" \
  ...
  # Wait for the first docker run, confirming the job subshell is live.
  deadline=$((SECONDS + 15))
  while true; do
    if [ -f "$run_log" ] && [ "$(wc -l < "$run_log" | tr -d '[:space:]')" -ge 1 ]; then
      break
    ...
    if ! kill -0 "$controller_pid" 2>/dev/null; then
      sed 's/^/  /' "${case_dir}/controller.log" >&2 || true
      fail "controller exited before first launch in SIGTERM subshell stop case"
    fi
    if [ "$SECONDS" -ge "$deadline" ]; then
      sed 's/^/  /' "${case_dir}/controller.log" >&2 || true
      fail "timed out waiting for first worker launch in SIGTERM subshell stop case"
    fi
    sleep 0.1
  done

  # Capture bash subshell PIDs before sending SIGTERM.
  local candidate_pid=""
  ...
  [ "${#subshell_pids[@]}" -gt 0 ] || fail "no bash job subshell PIDs found in SIGTERM subshell stop case"

  kill -TERM "$controller_pid"

  # Controller must exit within 10 seconds — well under the 30s mock docker
  # wait — proving the subshell was stopped, not waited for.
  deadline=$((SECONDS + 10))
  while kill -0 "$controller_pid" 2>/dev/null; do
    if [ "$SECONDS" -ge "$deadline" ]; then
      kill -KILL -- -"$controller_pid" 2>/dev/null || true
      fail "controller did not exit within 10s after SIGTERM in subshell stop case"
    fi
    sleep 0.1
  done
  ...
  # Kill any remaining members of the setsid process group so the mock sleep
  # does not leak into subsequent test cases.
  kill -KILL -- -"$controller_pid" 2>/dev/null || true

  # All tracked job subshell PIDs must be gone.
  orphan_count=0
  ...
  assert_eq "0" "$orphan_count" "tracked job subshells must be stopped by SIGTERM shutdown path"

  echo "PASS: SIGTERM stops tracked job subshells via handle_shutdown (controller_exit=${controller_status})"
}
...
run_shutdown_term_stops_job_subshells_case "$repo_root" "${tmpdir}/shutdown-term-subshells"
